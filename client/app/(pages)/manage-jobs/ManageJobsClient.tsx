...
function getMessage(response: JobsResponse | JobDetailResponse) {
...
function getJobId(job: JobSummary) {
...
function getFormFromJob(job: JobSummary): JobFormState {
  return {
    title: job.title ?? "",
...
export default function ManageJobsClient() {
  ...
  const [pagination, setPagination] = useState<JobsResponse["pagination"]>();
  const [searchInput, setSearchInput] = useState("");
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const [sortBy, setSortBy] = useState("updatedAt");
  const [sortType, setSortType] = useState<SortType>("dsc");
  ...
  const [jobPendingDelete, setJobPendingDelete] = useState<JobSummary | null>(
    null,
  );
  ...
  const selectedJob = useMemo(
    () => jobs.find((job) => getJobId(job) === editingJobId),
    [editingJobId, jobs],
  );
  ...
  async function handleEdit(job: JobSummary) {
  ...
  function handleDelete(job: JobSummary) {
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          ...
          <button
            type="button"
            onClick={resetForm}
            className="site-button inline-flex items-center justify-center gap-2 rounded-md px-4 py-2 text-sm font-semibold transition"
          >
            <Icon name="plus" />
            New Job
          </button>
                ...
                <tbody>
                  {isLoading ? (
                    <tr>
                      <td
                        colSpan={5}
                        className="site-muted px-4 py-10 text-center"
                      >
                        Loading jobs...
                      </td>
                    </tr>
                          ...
                          <td className="px-4 py-4 align-top">
                            <div className="flex justify-end gap-2">
                              ...
                              <button
                                ...
                                className="rounded-md border border-[var(--site-danger-border)] bg-[var(--site-danger-bg)] px-3 py-1.5 text-xs font-semibold text-[var(--site-danger-text)] disabled:opacity-60"
                              >
                                {deletingJobId === jobId
                                  ? "Deleting"
                                  : "Delete"}
              ...
              <form onSubmit={handleSubmit} className="space-y-4 p-4">
                  ...
                  <label className="block">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setForm((current) => ({
                          ...current,
                          jobType: event.target
                            .value as JobFormState["jobType"],
                        }))
                      }
                      className="site-field mt-1 w-full rounded-md border px-3 py-2 text-sm focus:outline-none"
                    >
                ...
                <label className="block">
                  ...
                  <textarea
                    ...
                    className="site-field mt-1 min-h-36 w-full resize-y rounded-md border px-3 py-2 text-sm leading-6 focus:outline-none"
                    maxLength={5000}
                    placeholder="Describe responsibilities, outcomes, and requirements."
                  />
                </label>
          ...
          <div className="site-border site-card w-full max-w-md rounded-lg border">
            ...
            <div className="flex flex-col-reverse gap-2 p-5 sm:flex-row sm:justify-end">
              <button
                ...
                disabled={deletingJobId === getJobId(jobPendingDelete)}
