    ...
    return (<section className="px-5 py-12 md:px-[8vw]">
      <div className="mx-auto max-w-6xl">
        ...
        {!result.ok ? (<div className="site-danger mt-6 rounded-lg border px-4 py-3 text-sm">
            {result.body.error ?? result.body.message ?? "Unable to load applications."}
          </div>) : null}
                ...
                <div className="grid gap-4 md:grid-cols-[1fr_180px] md:items-start">
                  <div>
                    ...
                    <p className="site-muted mt-1 text-sm">
                      {job.location ?? "Location not set"} · Applied {formatDate(application.createdAt)}
                    </p>
                    {application.coverLetter ? (<p className="site-muted mt-3 line-clamp-2 text-sm leading-6">
                        {application.coverLetter}
                      </p>) : null}
                  </div>
                  <span className={`w-fit rounded-md border px-3 py-1.5 text-xs font-semibold ${getStatusClass(application.status)}`}>
